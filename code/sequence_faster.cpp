...
RollingMedian(multiset<long long> ml){

    order = ml;

    it = order.begin();
    advance(it, order.size()/2);
}
...
RollingMedian* Merge(RollingMedian* &r1, RollingMedian* &r2){

    multiset<long long> ml;

    merge(  (r1->order).begin(), (r1->order).end(),
            (r2->order).begin(), (r2->order).end(),
            inserter(ml, ml.begin()));

    delete r1;
    delete r2;

    return ( new RollingMedian(ml) );
}
